...
async function main(inputFile) {
  ...
  const allCardsExceptJoker = Object.keys(cardValueMap).filter((card) => card !== "J");
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    let details = { typeValue:0 }
    let testDetails;

    // try all cards for joker and get the best hand
    for (let card of allCardsExceptJoker) {
      testDetails = sortCardsInHand(hands[i].cards.map((c) => (c === "J" ? card : c)));
      if (testDetails.typeValue > details.typeValue) {
        details = testDetails;

        // preserve joker card value
        details.cardValues = hands[i].cards.map((card) => cardValueMap[card]);
      }
    }
    
    hands[i].details = JSON.parse(JSON.stringify(details));
  }
